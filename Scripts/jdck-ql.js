...
// prettier-ignore
function QingLong(t, e, n) {
  const o = (t, e = "GET") => {
    if ($.isNode() && t.hasOwnProperty("use_proxy") && t.use_proxy) {
      require("dotenv").config();
      const e = process.env.PROXY_HOST || "127.0.0.1",
        n = process.env.PROXY_PORT || 7890,
        o = require("tunnel"),
        s = { https: o.httpsOverHttp({ proxy: { host: e, port: 1 * n } }) };
      Object.assign(t, { agent: s });
    }
  ...
  return new (class {
    ...
    async checkLogin() {
      let t;
      try {
        t = JSON.parse($.getdata("yuheng_ql_token") || "{}");
      } catch (t) {
        return (
      ...
      if (Object.keys(t).length > 0) {
        const { token: e, expiration: n } = t,
          o = new Date().getTime();
        o > n
          ? ($.log("❌The token has expired"), await this.getAuthToken())
          : ((this.token = e),
            $.log(
    ...
    async getAuthToken() {
      ...
      try {
        console.log(`传入参数: ${JSON.stringify(t)}`);
        const { code: e, data: n, message: s } = await o(t);
        if (200 !== e) throw s || "Failed to obtain user token.";
        {
          const { token: t, token_type: e, expiration: o } = n;
          $.log(
            `✅The token is successfully obtained: ${t} and is valid until ${$.time(
              "yyyy-MM-dd HH:mm:ss",
              1e3 * o
            )}`
          ),
            (this.token = `${e} ${t}`),
            $.setdata(
              JSON.stringify({ token: this.token, expiration: 1e3 * o }),
              "yuheng_ql_token"
            );
        }
      } catch (t) {
    ...
    async getEnvs() {
      ...
      try {
        const { code: e, data: n, message: s } = await o(t);
        if (200 !== e) throw s || "Failed to obtain the environment variable.";
        (this.envs = n), $.log("✅Obtaining environment variables succeeded.");
      } catch (t) {
        throw t
          ? "object" == typeof t
            ? JSON.stringify(t)
    ...
    checkEnvByName(t) {
      return this.envs.findIndex((e) => e.name === t);
    }
    checkEnvByRemarks(t) {
      return this.envs.findIndex((e) => e.remarks === t);
    }
    checkEnvByValue(t, e) {
      const n = t.match(e);
      if (n) {
        const e = this.envs.findIndex((t) => t.value.includes(n[0]));
        return e > -1
          ? ($.log(`🆗${t} Matched: ${n[0]}`), e)
          : ($.log(`⭕${t} No Matched`), -1);
      }
      return $.log(`⭕${t} No Matched`), -1;
    }
    async addEnv(t) {
      const e = {
        url: `${this.host}open/envs`,
      ...
      try {
        const { code: t, message: n } = await o(e, "post");
        if (200 !== t) throw n || "Failed to add the environment variable.";
    ...
    async updateEnv(t) {
      ...
      try {
        const { code: n, message: s } = await o(e, "post");
        if (200 !== n) throw s || "Failed to update the environment variable.";
        $.log("✅The environment variable was updated successfully."),
          await this.enableEnv([t._id]);
      } catch (t) {
        throw t
          ? "object" == typeof t
            ? JSON.stringify(t)
    ...
    async deleteEnv(t) {
      ...
      try {
        const { code: t, message: n } = await o(e, "post");
        if (200 !== t) throw n || "Failed to delete the environment variable.";
    ...
    async enableEnv(t) {
      ...
      try {
        const { code: t, message: n } = await o(e, "post");
        if (200 !== t) throw n || "Failed to enable the environment variable.";
    ...
    async getEnvById(t) {
      const e = {
        url: `${this.host}open/envs/${t}`,
        headers: { Authorization: `${this.token}` },
      };
      try {
        const { code: t, data: n, message: s } = await o(e);
        if (200 === t) return n;
        throw s || "Failed to get the environment variable.";
      } catch (t) {
        throw t
          ? "object" == typeof t
            ? JSON.stringify(t)
