...
// prettier-ignore
function Tools(scriptName) {
  (this.startTime = new Date().getTime()),
    (this.name = scriptName),
    (this.isQuanX = "undefined" != typeof $task),
    (this.isLoon = "undefined" != typeof $loon),
    (this.isSurge =
      "undefined" != typeof $httpClient && "undefined" != typeof $utils),
    (this.isNode = "function" == typeof require),
    (this.log = (...logs) => console.log([...logs].join("\n"))),
    (this.getEnv = () =>
      this.isQuanX
        ? "QuanX"
        : this.isSurge
    ...
    (this.notify = (
      ...
      isQuanX && $notify(title, subtitle, content, options),
        isSurge &&
          $notification.post(
            title,
            subtitle,
    ...
    (this.done = (val = {}) => {
      const endTime = new Date().getTime(),
        costTime = (endTime - this.startTime) / 1e3;
      switch (
        (this.log("", `🔔${this.name}, 结束! 🕛 ${costTime} 秒`), this.getEnv())
      ) {
        case "QuanX":
        case "Surge":
        case "Loon":
