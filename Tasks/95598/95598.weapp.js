...
function WSGW() {
  return new (class {
    async getBaseInfo() {
      const t = $request.headers,
        e = t.Authorization || t.authorization;
      WSGW_DEBUG && $.log(`🔐 Authorization: ${e}`);
      const a = t.t;
      WSGW_DEBUG && $.log(`🔐 t: ${a}`);
      try {
        const t = {
            url: MY_SERVER + "/api/get_openid",
            method: "POST",
            headers: { auth: e, token: a },
            body: {},
          },
          { data: o } = await fetchData(t),
          { mobile_dst: n, openId: i, nickname: r, photo: s } = o;
        WSGW_DEBUG && $.log(`🔐 用户标识: ${i}`),
          $.setdata(i, "wsgw_openid"),
          $.setdata(a, "wsgw_token"),
          $.setdata(e, "wsgw_auth");
        const c = i.slice(0, 3) + "****" + i.slice(-4);
    ...
    async initLogin() {
      ...
      try {
        const t = await fetchData({
          url: MY_SERVER + "/api/oauth_http_api",
          method: "POST",
          dataType: "json",
          body: {
            yuheng: {
              quInfo: {
                thirdPartnerId: WSGW_OPENID,
                thirdUnionId: WSGW_OPENID,
                accountType: WSGW_CONFIG.accountType,
              },
              uscInfo: { member: WSGW_CONFIG.channelCode, proNo: "00000" },
              client_id: WSGW_CONFIG.client_id,
              client_secret: WSGW_CONFIG.client_screct,
            },
          },
        });
        if (!t?.body) throw "⚠ 初始化登录加密数据失败";
        const e = {
            url: WSGW_CONFIG.base_url + "/osg-open-uc0001/outer/c03/initLogin",
            method: "post",
            dataType: "json",
            headers: {
              wsgwType: "mnpg",
              ...
              client_id: WSGW_CONFIG.client_id,
              t: WSGW_TOKEN,
              Authorization: WSGW_AUTH,
            },
            body: t.body,
          },
          a = await fetchData(e),
          o = await fetchData({
            url: MY_SERVER + "/api/decrypt_new",
            method: "POST",
            dataType: "json",
            body: { yuheng: a.encryptData, RSI: t.randomkey },
          });
        if (!o?.bizrt?.token) throw "初始化登录解密失败";
        const {
          bizrt: n,
          srvrt: { resultCode: i, resultMessage: r },
        } = o;
        if ("0000" !== i) throw `初始化登录失败: ${r}`;
        (WSGW_TOKEN = n.token),
          $.log(`🎉 初始化登录成功: ${n.token}`),
          $.setdata(n.token, "wsgw_token");
      } catch (t) {
        throw `初始化登录异常: ${t}`;
      } finally {
        $.log("⏳ 初始化登录结束");
    ...
    async getAuthCode() {
      ...
      try {
        const t = new Date().getTime(),
          e = await fetchData({
            url: MY_SERVER + "/api/get_sign",
            method: "POST",
            dataType: "json",
            body: { yuheng: `${WSGW_CONFIG.client_id}${t}` },
          }),
          a = {
            url: WSGW_CONFIG.base_url + "/oauth2/oauth/authorize",
            headers: {
              xweb_xhr: 1,
              ...
              referer:
                "https://servicewechat.com/wx5899bdb8721621d6/47/page-frame.html",
            },
            body: {
              client_id: WSGW_CONFIG.client_id,
              response_type: "code",
              redirect_url: "/OPF_TRQG5UQN/OPF_M0K4URCA/index.html",
              timestamp: t,
              rsi: WSGW_TOKEN,
              sign: e,
            },
          ...
          n = await fetchData({
            ...
            method: "POST",
            dataType: "json",
            body: { yuheng: o.data, RSI: WSGW_TOKEN },
          });
        if (!n) throw "获取授权码解密失败";
        const { code: i, message: r, data: s } = n;
        if ("1" != i) throw `获取授权码失败: ${r}`;
        (this.authCode = s.redirect_url.split("code=")[1]),
          $.log(`🎉 获取授权码成功: ${this.authCode}`);
      } catch (t) {
        throw `❌ 获取授权码异常: ${t}`;
      } finally {
        $.log("⏳ 获取授权码结束");
    ...
    async getMiniPGToken() {
      $.log("⏳ 开始刷新授权...");
      try {
        const t = WSGW_CONFIG.base_url + "/oauth2/oauth/getMiniPGToken",
          e = new Date().getTime(),
          a = await fetchData({
            url: MY_SERVER + "/api/get_sign",
            method: "POST",
            dataType: "json",
            body: { yuheng: `${WSGW_CONFIG.client_id}${e}` },
          }),
          o = {
            url: t,
            body: {
          ...
          n = await fetchData(o),
          { code: i, message: r, data: s } = n;
        if ("1" != i) throw `刷新授权失败: ${r}`;
        $.log(`🎉 刷新授权成功: ${s.access_token}`),
          (WSGW_AUTH = `Bearer ${s.access_token}`),
          $.setdata(WSGW_AUTH, "wsgw_auth");
      } catch (t) {
        throw `❌ 刷新授权异常: ${t}`;
      } finally {
        $.log("⏳ 刷新授权结束");
    ...
    async getUserInfo() {
      ...
      try {
        const t =
            WSGW_CONFIG.base_url + "/osg-open-uc0001/member/arg/020360030",
          e = await fetchData({
            url: MY_SERVER + "/api/ph_http_api",
            dataType: "json",
            body: {
              yuheng: {
                token: WSGW_TOKEN,
                uscInfo: {
                  tenant: "state_grid",
                  member: WSGW_CONFIG.channelCode,
                  devciceId: "",
                  devciceIp: "",
                },
                quInfo: {
        ...
        const a = {
          ...
          n = await fetchData({
            url: MY_SERVER + "/api/decrypt_new",
            dataType: "json",
            body: { yuheng: o.encryptData, RSI: e.randomkey },
          });
        if (!n?.bizrt) throw "获取用户信息解密失败";
        const {
          bizrt: i,
          srvrt: { resultCode: r, resultMessage: s },
        } = n;
        if ("0000" !== r) throw `获取用户信息失败: ${s}`;
        {
          const { userInfo: t } = i;
          ($.userInfo = t),
            WSGW_DEBUG &&
              ...
              $.log($.toStr(t)),
              $.log("====================================")),
            $.log(`🎉 获取用户信息成功: ${$.userInfo.nickname}`);
        }
      } catch (t) {
      } finally {
        $.log("⏳ 获取用户信息结束");
        ...
        o =
          t && e
            ? {
                ...
                PROVINCE_CODE: e,
                POWER_USER_ID: t,
                SENSITIVE_POWER_USER_ID: t,
                timestamp: a,
              }
            : { TYPE: "0", timestamp: a };
      $.log("⏳ 开始获取电费信息...");
      try {
        const t = WSGW_CONFIG.base_url + "/osg-open-p0001/member/c5/f05",
          e = await fetchData({
            url: MY_SERVER + "/api/oauth_http_api",
            dataType: "json",
            body: {
              yuheng: {
                ...
                data: {
                  common: {
                    ...
                    WSGW_PLAT: "6",
                    REQUEST_SOURCE: "10",
                    SDKVERSION: "1.2024.03.08.01",
                    TOKEN: WSGW_TOKEN,
                    token: WSGW_TOKEN,
                  },
                  data: o,
                },
                _t: WSGW_TOKEN.substring(WSGW_TOKEN.length / 2),
              },
            },
          });
        if (!e?.body) throw "获取电费信息加密数据失败";
        const a = {
            url: t,
            dataType: "json",
            headers: {
          ...
          i = await fetchData({
            url: MY_SERVER + "/api/decrypt_new",
            dataType: "json",
            body: { yuheng: n.encryptData, RSI: e.randomkey },
          });
        if (!i?.data) throw "获取电费信息解密失败";
        const { code: r, message: s, data: c } = i;
        if (1 === r) {
          const {
            data: { ELEC_TYPE_LIST: t },
          } = c;
          return (
            WSGW_DEBUG &&
              ($.log("===================================="),
              $.log("电费信息:"),
              $.log($.toStr(t[0].ARREARS_LIST)),
              $.log("====================================")),
            $.log("🎉 获取电费信息成功"),
            t[0].ARREARS_LIST
          );
    ...
    async getElectricityUsage(t, e) {
      $.log("⏳ 开始查询用电量信息...");
      try {
        const a =
            WSGW_CONFIG.base_url + "/osg-open-bc0001/member/arg/010070001",
          o = new Date().getTime(),
          n = {
            data: {
              serialNo: o,
              srvCode: "emss-bia-bill-front/member/c31/f01",
              channelCode: WSGW_CONFIG.channelCode,
              funcCode: "11",
              acctId: "manager",
              userName: "111",
              promotType: "1",
        ...
        const r = {
          ...
          c = await fetchData({
            url: MY_SERVER + "/api/decrypt_new",
            dataType: "json",
            body: { yuheng: s.encryptData, RSI: i.randomkey },
          });
        if (!c?.data) throw "查询用电量信息解密失败";
        const { code: d, message: _, data: h } = c;
        if (1 === d)
          return (
            WSGW_DEBUG &&
              ($.log("===================================="),
...
async function fetchData(e) {
  if (("string" == typeof e && (e = { url: e }), !e?.url))
  ...
  try {
    ...
    const y = r ? r.toLowerCase() : n ? "post" : "get";
    (o += "post" === y && i ? `?${$.queryStr(i)}` : ""),
      (t = s),
      (s = Object.fromEntries(
        Object.entries(t).map(([e, t]) => [e.toLowerCase(), t])
      ));
    const h =
      "pc" === p
        ? "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.36 Edge/16.16299"
        : "Mozilla/5.0 (iPhone; CPU iPhone OS 14_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.0 Mobile/15E148 Safari/604.1";
    (s["user-agent"] = s["user-agent"] || h),
      "json" === a && (s["content-type"] = "application/json;charset=UTF-8");
    const f = {
      ...e,
      url: o,
      method: y,
      headers: s,
      opts: m,
      timeout: $.isSurge() ? l / 1e3 : l,
    };
    if (("get" === y && i && (f.params = i), "post" === y)) {
      const e = "json" === a ? $.toStr : $.queryStr;
      f.body = (n && ("object" == typeof n ? e(n) : n)) || n;
    }
    if ($.isNode() && u) {
      const { PROXY_HOST: e = "127.0.0.1", PROXY_PORT: t = 7890 } = process.env,
        o = require("tunnel");
      f.agent = { https: o.httpsOverHttp({ proxy: { host: e, port: +t } }) };
    }
    const w = new Promise((e, t) => {
      $[y](f, (r, s, n) => {
        if (r) {
          const e = n ? `${$.toObj(n)?.message || n}` : "";
          console.log(`请求接口: ${o} 异常: ${e}`), t(new Error(e));
        } else e("response" === c ? s : $.toObj(n) || n);
      });
    });
    return $.isQuanX()
      ? await Promise.race([
          w,
          new Promise((e, t) =>
            setTimeout(() => t(new Error("网络开小差了~")), l)
          ),
        ])
      : w;
  } catch (e) {
 ...
 */
// prettier-ignore
async function showNotification(n, o = "", i = "", t = {}) {
  const e = "undefined" != typeof $app && "undefined" != typeof $http,
    s = t["open-url"],
    f = t["media-url"];
  if (
    ($.isQuanX() && $notify(n, o, i, t),
    $.isSurge() && $notification.post(n, o, i, { url: s }),
    $.isLoon())
  ) {
    const t = {},
      $ = $loon.split(" ")[1].split(".")[0];
    s && (t.openUrl = s),
      f && 16 !== Number($) && (t.mediaUrl = f),
      "{}" === JSON.stringify(t)
        ? $notification.post(n, o, i)
        : $notification.post(n, o, i, t);
  }
  const c = `${i}${s ? `\n点击跳转: ${s}` : ""}${f ? `\n多媒体: ${f}` : ""}`;
  if (e) {
    require("push").schedule({ title: n, body: `${o ? `${o}\n` : ""}${c}` });
  }
  if ($.isNode())
    try {
      await notify.sendNotify(`${n}\n${o}`, c);
    } catch (n) {
      console.log("没有找到sendNotify.js文件 不发送通知");
    }
  console.log(`${n}\n${o}\n${c}\n`);
}
// prettier-ignore
