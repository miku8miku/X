...
async function HD4K() {
  ...
  const getDetail = async (url, title) => {
    ...
    try {
        ...
        const _$ = $.cheerio.load(body);
        let images = _$('img[loading="lazy"][decoding="async"]')
           .map((_, el) => _$(el).attr("src"))
           .get()
           .filter((it) => it.match(/webp\?w=\d+$/));

      // 查找所有链接
        const allLinks = _$('a').map((_, el) => _$(el).attr('href')).get();
        const regex = /\.html\/(\d+)/;
        const matchedLinks = [];
        ...
        for (const link of matchedLinks) {
            try {
                const linkResponse = await $.http.get(link);
                const link$ = $.cheerio.load(linkResponse.body);
